...
@section('content')
    <div class="container-fluid">
            ...
            <div class="card-body">
                <form action="{{ route('penugasan.index') }}" method="GET">
                    <div class="row g-3 align-items-end">
                        ...
                        <div class="col-md-4">
                            {{-- PERBAIKAN: Menghapus pilihan "Semua Semester" --}}
                            ...
                            <select name="jenis_semester" id="jenis_semester" class="form-select"
                                ...
                                <option value="gasal" {{ ($jenisSemester ?? 'gasal') == 'gasal' ? 'selected' : '' }}>Gasal
                                </option>
                                <option value="genap" {{ ($jenisSemester ?? '') == 'genap' ? 'selected' : '' }}>Genap
                                </option>
                            </select>
                        </div>
                    </div>
        ...
        <form action="{{ route('penugasan.store') }}" method="POST">
            @csrf
            <input type="hidden" name="jenis_semester" value="{{ $jenisSemester ?? 'gasal' }}">
                                    ...
                                    <td>
                                        <div class="input-group">
                                            <input type="text" class="form-control search-dosen-input"
                                                ...
                                                id="search_dosen_{{ $mk->id }}_{{ $kelas->id }}">
                                            <select name="assignments[{{ $mk->id }}_{{ $kelas->id }}][dosen_id]"
                                                class="form-select dosen-select"
                                                id="dosen_select_{{ $mk->id }}_{{ $kelas->id }}">
                                        ...
                                        <input type="hidden"
                                            name="assignments[{{ $mk->id }}_{{ $kelas->id }}][mata_kuliah_id]"
                                            value="{{ $mk->id }}">
                                        <input type="hidden"
                                            name="assignments[{{ $mk->id }}_{{ $kelas->id }}][kelas_id]"
                                            value="{{ $kelas->id }}">
                                    </td>
