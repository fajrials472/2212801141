...
class JadwalService
{
    ...
    {
        // PERBAIKAN: Logika baru untuk mengarsipkan jadwal lama
        $jadwalAktif = DB::table('jadwal')->get();

        if ($jadwalAktif->isNotEmpty()) {
            // 1. Buat record versi arsip baru
            $namaVersi = 'Arsip Jadwal ' . now()->format('d M Y H:i:s');
            ...
            // 2. Salin jadwal aktif ke tabel arsip
            $jadwalUntukArsip = $jadwalAktif->map(function ($item) use ($versionId) {
                return [
        ...
        // 3. Lanjutkan proses generate jadwal seperti biasa
        set_time_limit(300);
        DB::table('jadwal')->truncate();

        // ... (sisa logika generateJadwal Anda tetap sama)
        ...
        while (!empty($assignmentQueue)) {
            ...
            foreach ($assignmentQueue as $item) {
                ...
                if (in_array($uniqueAssignmentKey, $scheduledAssignments, true)) {
                    continue;
                }
                ...
                $hariKuliah = in_array($tipeKelas, ['A', 'B', 'C', 'D', 'E'], true)
                    ? $this->hariKerjaNormal
                    : $this->hariKuliahKelasF;

                if (!isset($dosenDays[$item->dosen_id])) {
                    $dosenDays[$item->dosen_id] = [];
                }
                ...
                foreach ($hariKuliah as $hari) {
                    ...
                    for ($pass = 1; $pass <= 2 && !$isScheduled; $pass++) {
                        ...
                        while ($jam < $jamSelesaiHarian) {
                            ...
                            if (
                                ($jam >= strtotime('12:00') && $jam < strtotime('13:00')) ||
                                ($jamSelesai > strtotime('12:00') && $jamSelesai <= strtotime('13:00'))
                            ) {
                                $jam = strtotime('13:00');
                                continue;
                            }

                            if ($jamSelesai > strtotime('17:30')) {
                                break;
                            }

                            $countHariIni = $scheduledCoursesPerDay[$item->kelas_id][$hari] ?? 0;
                            $limitPerHari = $this->maxCoursesPerDay[$tipeKelas] ?? 2;
                            if ($countHariIni >= $limitPerHari) {
                                break;
                            }
            ...
            if (empty($currentQueue) || count($currentQueue) === count($assignmentQueue)) {
                break;
            }
            $assignmentQueue = $currentQueue;
        }

        if (!empty($assignmentQueue)) {
            Log::info('Penugasan yang gagal dijadwalkan (sisa):', $assignmentQueue);
        }
    ...
    private function isSlotTersedia($hari, $mulai, $selesai, $dosenId, $ruanganId, $kelasId, $jadwalDosen, $jadwalRuangan, $jadwalKelas)
    {
        if (isset($jadwalKelas[$kelasId][$hari])) {
            foreach ($jadwalKelas[$kelasId][$hari] as $jadwal) {
                if (max($mulai, $jadwal['mulai']) < min($selesai, $jadwal['selesai'])) return false;
            }
        }
        if (isset($jadwalDosen[$dosenId])) {
            foreach ($jadwalDosen[$dosenId] as $jadwal) {
                if ($jadwal['hari'] == $hari && max($mulai, $jadwal['mulai']) < min($selesai, $jadwal['selesai'])) return false;
            }
        }
